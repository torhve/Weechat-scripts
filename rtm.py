...
def test(apiKey, secret, token=None):
    ...
    rspTasks = rtm.tasks.getList(list_id="3290931", filter='status:incomplete')
    #r = rtm.settings.getList().settings
    #print r.defaultlist
    #for a in r:
    #    print a
    import datetime
    for item in rspTasks.tasks.list.taskseries:
         print item.name, item.task.priority
    #print dir(rspTasks.tasks.list)#.taskseries
...
if __name__ == '__main__':
    ...
    token = 'a500cb41aaa2c9b9245f44a22ef8fc3ab3f9a660'
    test(apik, secret, token)
